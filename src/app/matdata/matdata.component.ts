...
@Component({
  selector: 'app-matdata',
  templateUrl: './matdata.component.html',
  styleUrls: ['./matdata.component.css']
})
export class MatdataComponent implements OnInit {
  ...
  dt = new Date();
  endDate : any = "2022-04-11";
  totalData : number = 0;
  recordPerPage : any = 10;
  currentPage : number = 1;
  totalPages : any = 1;
  fileName = "matdata.xlsx";
  exportedata: any = [];
  ...
  ngOnInit(): void {
    ...
    if(this.cookieValue == ""){
      ...
      this.service.getUserMatDuration(this.startDate,this.endDate,this.currentPage  , this.search ,'' ).subscribe(res=>{
        this.resp = res;
  ...
  searchRecord(){
    ...
    this.service.getUserMatDuration(this.startDate,this.endDate,this.currentPage  , this.search ,keyword ).subscribe(res=>{
      this.resp = res;
      if(this.resp.code == 200){
        this.matInfo = this.resp.data;
        console.log(this.matInfo);
  ...
  exportexcel(){
    ...
    if(keyword != ''){
      ...
      for (let index = 1; index <= this.totalPages; index++) {
        ...
        this.service.getUserMatDuration(this.startDate,this.endDate, index, this.search ,keyword ).subscribe(res=>{
          this.resp = res;
